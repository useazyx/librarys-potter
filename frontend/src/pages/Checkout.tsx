...
export default function Checkout() {
    ...
    return (
      <section className="mx-auto flex min-h-[80vh] max-w-2xl flex-col items-center justify-center px-6 py-32 text-center">
        <motion.div
          ...
          className="mb-8 rounded-full bg-sage-600/15 p-6 text-sage-400"
        >
          <CheckCircle2 size={54} aria-hidden />
        </motion.div>

        <p className="eyebrow mb-3">A coruja já partiu</p>
        <h1 className="font-display text-4xl text-parchment-50 sm:text-5xl">Pedido {placed.code} confirmado</h1>

        <p className="mt-5 text-parchment-200/80">
          {placed.items.reduce((sum, item) => sum + item.quantity, 0)} livro(s) a caminho de{' '}
          <strong className="text-parchment-50">{placed.delivery.city}</strong>. Total de{' '}
          <strong className="text-gold-400">{formatPrice(placed.total)}</strong>.
        </p>

        <div className="mt-10 flex flex-wrap justify-center gap-4">
          <ButtonLink to="/perfil" size="lg" variant="gold">
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <section className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center px-6 text-center">
        <h1 className="font-display text-4xl text-parchment-50">Nada para finalizar</h1>
        <p className="mt-4 text-parchment-200/70">Seu carrinho está vazio.</p>
        <div className="mt-10">
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 pb-24 pt-36 lg:px-10">
      ...
      <h1 className="mb-12 font-display text-4xl text-parchment-50 sm:text-5xl">Finalizar compra</h1>
      ...
      <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr]">
        <form onSubmit={placeOrder} className="surface-paper rounded-2xl p-8 shadow-book lg:p-10" noValidate>
          <h2 className="mb-6 flex items-center gap-2 font-display text-2xl">
            <Truck size={20} className="text-burgundy-600" aria-hidden />
            Entrega
          </h2>
          ...
          {errors.form && (
            <p className="mt-6 rounded-lg bg-burgundy-500/10 px-4 py-3 text-sm text-burgundy-600" role="alert">
              {errors.form}
            </p>
          )}
          ...
          <p className="mt-4 text-xs text-night-800/55">
        ...
        <aside className="lg:sticky lg:top-28 lg:h-fit">
          <div className="rounded-2xl border border-parchment-100/10 bg-night-800/70 p-8">
            <h2 className="font-display text-2xl text-parchment-50">Seu pedido</h2>
              ...
              {cart.items.map((line) => (
                <li key={line.id} className="flex gap-4">
                  ...
                  <div className="min-w-0 flex-1 text-sm">
                    <p className="text-parchment-100">{line.book.title}</p>
                    <p className="text-parchment-300/60">
                      {line.quantity}× {formatPrice(line.book.price)}
                    </p>
                  </div>
                  <span className="shrink-0 text-sm text-parchment-100">{formatPrice(line.lineTotal)}</span>
                </li>
              ))}
            </ul>

            <dl className="mt-6 space-y-3 border-t border-parchment-100/10 pt-6 text-sm">
              <div className="flex justify-between">
                <dt className="text-parchment-300/70">Subtotal</dt>
                <dd className="text-parchment-100">{formatPrice(cart.subtotal)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-parchment-300/70">Frete</dt>
            ...
            <div className="mt-5 flex items-end justify-between border-t border-parchment-100/10 pt-5">
              <span className="text-[0.68rem] uppercase tracking-[0.22em] text-parchment-300/70">Total</span>
